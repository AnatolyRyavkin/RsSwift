...
//        -> Disposable {
//            let clouser: (Event<Element>) -> Void = { a in
//                print("a - from extension ObservableType let clouser: (Event<Element>) -> Void = { a in - ",a.element!)
//                on(a)
//                on(.next(a.element!))
//                print("a - from extension ObservableType let clouser: (Event<Element>) -> Void = { a in - ",a)
//            }
//
//            let observer = AnonymousObserver(clouser)
//            print("observer from Extensions = ",observer)
//
...
//    }
     ...
     public func subscribe(_ on: @escaping (Event<Element>) -> Void)
            -> Disposable {
                let clouser: (Event<Element>) -> Void = { a in
                    //print("whole clouser that launh clouser from asObserable ; Event<Element> = ",a)
                    on(a)
                }
                let observer = AnonymousObserver(clouser)
                //clouser(Event.next("clouser-TextEmiting" as! Element))
                //observer.onNext("observer-TextEmiting" as! Element)
                return self.asObservable().subscribe(observer)
        }
...
//        -> Disposable {
//
//            let observer = AnonymousObserver{ a in
//                on(a)
//            }
//            return self.asObservable().subscribe(observer)
//    }
